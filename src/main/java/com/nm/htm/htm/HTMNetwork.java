...
public class HTMNetwork implements Serializable {
    ...
    public static final String STR_DT = "DT";
    public static final String STR_MEASUREMENT = "Measurement";
    public static final String STR_DT_FORMAT = "YY-MM-dd HH:mm";
    ...
    public void createNetwork() throws Exception {
        if (id == null) {
            throw new Exception("Wrong behaviour, HTMNetwork id is uninitialized");
        }
        ...
        if (network == null) {
            ...
            network = Network.create(id, parameters)
    ...
    public List<ResultState> compute(List<Map<String, Object>> inputs) {
        HTMSubscriber htmSubscriber = new HTMSubscriber();
        Subscription subscription = network.observe().subscribe(htmSubscriber);
        for(Map<String, Object> input: inputs) {
            network.compute(input);
        }
        subscription.unsubscribe();
        return htmSubscriber.getStates();
    }
    
    private static Map<String, Map<String, Object>> setupMap(Map<String, Map<String, Object>> map, Integer n, Integer w, Double min, Double max, Double radius, Double resolution, Boolean periodic, Boolean clip, Boolean forced, String fieldName, String fieldType, String encoderType) {
        if (map == null) {
            map = new HashMap<>();
        }
    ...
    private static Map<String, Map<String, Object>> getFieldEncodingMap() {
    ...
    private static Map<String, Class<? extends Classifier>> getInferredFieldsMap(String field, Class<? extends Classifier> classifier) {
        final Map<String, Class<? extends Classifier>> inferredFieldsMap = new HashMap<>();
        inferredFieldsMap.put(field, classifier);
        return inferredFieldsMap;
    }

    private static Parameters getNetworkParams() {
        final Parameters p = Parameters.getAllDefaultParameters();
